...
export const REQUIRED_FIELDS: (keyof ProviderData)[] = [
  "first_name",
  "last_name",
  "email_address",
];
...
export default function ProviderForm({ onSubmit }: ProviderFormProps) {
  ...
  const isValid = (key: keyof ProviderData) => {
    ...
    if (!REQUIRED_FIELDS.includes(key) && fields[key].trim() === "")
      return true;
    return PROVIDER_INPUT_REGEXS[key].test(fields[key]);
  };
            ...
            {inputFields.map((key) => (
              <div key={key} className="input-group mb-3">
                <RegexInput
                  ...
                  regex={PROVIDER_INPUT_REGEXS[key]}
                  errorMessage={
                    errorShown(key) ? PROVIDER_INPUT_ERROR_MESSAGES[key] : ""
                  }
